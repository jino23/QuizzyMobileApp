...
class _ReviewGameScreenState extends State<ReviewGameScreen> {
  ...
  // We tried using HTTP requests to fetch the questions from PlayGame.dart and display it on the Review section, however it does not work.
  // Future<void> _loadSavedGames() async {
  //   final url = Uri.parse('https://apiendpoint.com/api/savedGames');
  //   final response = await http.get(url);
  //
  //   if (response.statusCode == 200) {
  //     final List<dynamic> data = jsonDecode(response.body);
  //     setState(() {
  //       savedGames = data.cast<Map<String, dynamic>>();
  //     });
  //   } else {
  ...
  //   if (game.containsKey('questions')) {
  //     setState(() {
  //       questions = game['questions'];
  //     });
  //   } else {
  //     final url = Uri.parse('https://apiendpoint.com/api/questions/${game['code']}');
  //     final response = await http.get(url);
  //
  //     if (response.statusCode == 200) {
  //       final List<dynamic> data = jsonDecode(response.body);
  ...
  //       });
  //     } else {
  //       print('Failed to load questions for game ${game['title']}');
  //     }
  //   }
  // }
